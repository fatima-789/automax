    ...
    if request.method == 'POST':
        ...
        if login_form.is_valid():
            ...
            if user is not None:
                ...
                messages.error(request, 'An error occured in logging. Try again later!')
        else:
            messages.error(request, 'An error occured in logging. Try again later!')
...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        if register_form.is_valid():
            ...
            messages.error(request, 'An error occured in Registering. Try again later!')
            return render(request, 'views/register.html', {'register_form': register_form})
...
class ProfileView(View):
    ...
    def post(self, request):
        ...
        if user_form.is_valid() and profile_form.is_valid() and location_form.is_valid():
            ...
            messages.error(request, f'An error occured in updating profile.')
        return render(request, 'views/profile.html', {'user_form': user_form, 
                                                      'profile_form': profile_form,
                                                        'location_form': location_form,
                                                        'user_listings': user_listings,
